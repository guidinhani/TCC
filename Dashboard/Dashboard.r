...
server <- function(input, output) {
  ...
  output$quantidade_egressos_estudos <- renderValueBox({
    valueBox(
      paste0(QuantidadeEgressosEstudos(flag()), " de ", TotalDeEgressos(sigla_curso())), "Egressos estudaram",
  ...
  output$quantidade_egressos_profissional <- renderValueBox({
    valueBox(
      paste0(QuantidadeEgressosProfissional(egressos()), " de ", TotalDeEgressos(sigla_curso())), "Egressos trabalham",
      icon = icon("address-book"),
      color = "light-blue"
    )
  })
